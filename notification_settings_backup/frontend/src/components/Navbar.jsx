  ...
  const handleNotificationItemClick = async (notification) => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        if (message.includes('지원')) {
          // 지원 관련 알림
          if (authState.userType === 'company') {
            // 기업회원: 지원 알림인 경우 해당 공고의 후보자 목록으로 이동
            console.log('지원 알림 감지됨');
            
            // 알림 객체에서 relatedPostId 사용
            let postId = notification.relatedPostId;
            console.log('알림 객체의 relatedPostId:', postId);
            
            // relatedPostId가 없으면 메시지에서 추출 시도
            if (!postId) {
              // 패턴 1: "공고 - {postId}" 형태
              const postIdMatch1 = message.match(/공고\s*-\s*(\d+)/);
              if (postIdMatch1) {
                postId = postIdMatch1[1];
                console.log('패턴 1로 공고 ID 추출:', postId);
              }
              
              // 패턴 2: "공고 {postId}" 형태
              const postIdMatch2 = message.match(/공고\s+(\d+)/);
              if (postIdMatch2) {
                postId = postIdMatch2[1];
                console.log('패턴 2로 공고 ID 추출:', postId);
              }
              ...
              if (!postId) {
                ...
                if (numberMatch) {
                  ...
                  console.log('패턴 3으로 공고 ID 추출:', postId);
                }
              }
            }
            ...
            if (postId) {
              // 기업 대시보드의 후보자 목록 탭으로 이동 (추가 지원자 필터 적용)
              const targetPath = `/company/dashboard?postId=${postId}&tab=candidates&filter=additional-applicants`;
              console.log('이동할 경로:', targetPath);
              navigate(targetPath);
            } else {
              // 공고 ID를 찾을 수 없는 경우 대시보드로
              console.log('공고 ID를 찾을 수 없어 대시보드로 이동');
              navigate('/company/dashboard');
            }
          } else {
            // 개인회원은 대시보드로
            console.log('개인회원으로 대시보드 이동');
            navigate('/candidate/dashboard');
          }
        } else if (message.includes('포트폴리오 분석')) {
          // 포트폴리오 분석 완료 - 개인회원은 대시보드
        ...
        <div className="auth-buttons desktop-only">
          {authState.token ? (
            <>
              ...
              <div className="notification-button" onClick={handleNotificationClick} ref={notificationRef}>
                                ...
                                <div 
                                  ...
                                  onMouseEnter={e => {
                                    ...
                                    e.currentTarget.style.boxShadow = isRead ? 
                                      ...
                                      '0 0 0 1px rgba(16, 185, 129, 0.4), 0 8px 25px rgba(16, 185, 129, 0.15)';
                                  }}
                                  onMouseLeave={e => {
